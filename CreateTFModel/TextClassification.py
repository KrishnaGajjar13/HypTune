...
def TextClassificationStudy(
    ...
    def objective(trial):
        if use_transformer:
            ...
            tokenized = tokenizer(texts, padding=True, truncation=True, max_length=max_sequence_length, return_tensors='tf')
            X_train, X_val, y_train, y_val = train_test_split(tokenized['input_ids'], labels, test_size=0.2, stratify=labels)
